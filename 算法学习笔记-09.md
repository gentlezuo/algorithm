...
public class So79 {
    public List<String> readBinaryWatch(int num) {
        ...
        for (int i = 0; i <=num; i++) {
			...
			//此时hourlist中是满足num_1==i的所有的数字组合
			//minutelist满足num_1==num-1的所有组合
            for(int h:hourList){
...
public class So79 {
    ...
    private void help(int[] weiht,int hour,int minute,int index,int num,List<String> res){
        if(hour>11||minute>59)
            return;
        if(num==0){
            res.add(hour+":" + (minute<10?"0"+minute: + minute));
        }else if(index<10&&num>0){
            help(weiht,hour, minute, index + 1, num, res);
            if (index < 4) {
                help(weiht,hour + weiht[index], minute, index + 1, num - 1, res);
            } else {
                help(weiht,hour, minute + weiht[index], index + 1, num - 1, res);
            }
        }
    }
}
